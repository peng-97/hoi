    ...
    draw(layer) {
        this.clusters.forEach(cluster => {
            ...
            if (cluster.count == 1) {
                ...
                //keep size 画外圈
                ctx.arc(cluster.center[0], cluster.center[1], symbol.getRadius(cluster.count)+1, 0, Math.PI * 2, true);
                ctx.fill();
                ctx.stroke();
                ctx.fillStyle = symbol.getInnerColor(cluster.count);
                ctx.beginPath(); //Start path
                //keep size 画内圈
                ctx.arc(cluster.center[0], cluster.center[1], symbol.getRadius(cluster.count), 0, Math.PI * 2, true);
                ctx.fill();
                ctx.stroke();
                ...
                ctx.fillText(symbol.getCount(cluster.count), cluster.center[0], cluster.center[1]);
                ctx.restore();
             }
        })
