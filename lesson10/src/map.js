...
export class Map extends SubObject {
    ...
    constructor(htmlId, options = {}) {
        ...
        // this.layers.push(this.graphics);
        this.setView(options.zoom, options.center, options.bound)
    };
    createLayerOfParent() {
        let baseLayerDiv = document.createElement("div", { "classs": "gis-baseLayer" });
        let layerDiv = document.createElement("div", { "classs": "gis-layer" });
        this.container.appendChild(layerDiv);
        this.container.appendChild(baseLayerDiv);
    }
    //客户端图层先来后到，insertIndex不生效
    ...
    refresh() {
        ...
        this.drawLayer(layers)

        // this.updateExtent();
        // // let dvas = document.createElement("canvas");
        // // dvas.height = this.height;
        // // dvas.width = this.width;
        // // let ctxc = dvas.getContext("2d");
        // // let matrix = this.ctx.getTransform();
        // // ctxc.setTransform(matrix.a, matrix.b, matrix.c, matrix.d, matrix.e, matrix.f);
        // // this.layers.forEach(layer => layer.ctx = ctxc);
        // // this.offcanvas = dvas;
        // let layers = this.layers.concat(this.graphics);
        // this.drawLayer(layers);
    ...
    //     this.ctx.setTransform(1, 0, 0, 1, 0, 0);
    //     this.ctx.clearRect(0 - this.crs.tileSize, 0 - this.crs.tileSize, this.mapCanvas.width + this.crs.tileSize, this.mapCanvas.height + this.crs.tileSize);
    //     // this.ctx.drawImage(this.offcanvas, 0 - this.crs.tileSize, 0 - this.crs.tileSize, this.mapCanvas.width + this.crs.tileSize, this.mapCanvas.height + this.crs.tileSize);
    //     listPara.forEach(oper => {
    //         if (oper) {
    //             if (oper.type == 1) {
    //                 if (oper.value) {
    //                     this.ctx[oper.name](...oper.value)
    //                 } else {
    //                     this.ctx[oper.name]()
    //                 }
    //             } else {
    ...
    onClick(event) {
        ...
        layers.filter(layer => layer._client).forEach(layer => {
            let gras = layer.contains(event.x, event.y, "click");
            if (gras.length > 0) {
                res.push(gras[0]);
            }
        })
