...
export class gridCentroidUnion extends Cluster{
    ...
    */
    ...
    createClusters(features, map) {
        ...
        grids=grids.filter(t => t.count > 0).reduce((acc,grid) => {
            let cgrid = acc.find(t => {
                return Math.sqrt(Math.pow(t.center[0] - grid.center[0],2) + Math.pow(t.center[1] - grid.center[1],2)) < this.distance;
            })
            if (cgrid) {
    ...
    getCenter(rings,map) {
        ...
        return [(matrix.a * center[0] + matrix.e),(matrix.d * center[1] + matrix.f)];
    }
}