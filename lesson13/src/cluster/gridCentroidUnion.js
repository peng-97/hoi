...
export class gridCentroidUnion extends Cluster{
    ...
    */
    ...
    createClusters(features, map) {
        ...
        //合并在容差范围内的网格要素 //
        let pDistacne = this.distance / map.matrix.a;//平面坐标的距离
        grids = grids.filter(t => t.count > 0).reduce((acc, grid) => {
            let cgrid = acc.find(t => {
                let center = map.projection.project(t.center);
                let pcenter = map.projection.project(grid.center);
                return Math.sqrt(Math.pow(center[0] - pcenter[0], 2) + Math.pow(center[1] - pcenter[1],2)) <pDistacne;
            })
            if (cgrid) {
