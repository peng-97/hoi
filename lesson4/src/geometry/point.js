...
import { PictureMarkerSymbol } from "../symbol/pictureMarkerSymbol.js";
        ...
        if (!symbol) {
         ...
         if (symbol.type == "simple-marker") {
            ...
            if (symbol.icon) { //有图标片的话
                ...
                ctx.beginPath();
                ctx.arc(matrix.a * projectPoint[0] + matrix.e - symbol.xoffset, matrix.d * projectPoint[1] + matrix.f - symbol.yoffset, symbol.size, 0, Math.PI * 2, true);
                ctx.closePath();
                ctx.fill()
            }
             ctx.stroke()
