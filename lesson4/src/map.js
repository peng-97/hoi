...
import { GraphicLayer } from "./layers/graphicLayer.js";
export class Map {
    graphics;
    ctx = null;
    mapCanvas = null;
    ...
    constructor(htmlId, options = {}) {
        ...
        this.graphics = new GraphicLayer();
        this.graphics.map = this;
        if (options instanceof Object) {
            this.setView(options.zoom, options.center, options.extent)
        }
    ...
    setView(zoom, center, extent) {
        ...
        const a = tileSize * Math.pow(2, this.zoom) / (this.bound.xMax - this.bound.xMin) * this.bound.xscale;
        const d = tileSize * Math.pow(2, this.zoom) / (this.bound.yMax - this.bound.yMin) * this.bound.yscale;
        //对于 ef
        //用 cnavas中心来表示  center
        //mapCanvaa.width/2=a*orgin_x+e;//矩阵坐标转换公式
        //  可得  e=mapCans.width/2-a*orgin_x
        const e = this.mapCanvas.width / 2 - a * origin[0];
        //同理
        const f = this.mapCanvas.height / 2 - d * origin[1];
