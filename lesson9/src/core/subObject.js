export class SubObject{
    ...
    */
     ...
     */
    off(event, handler) {
        if (this._handers[event] && this._handers[event] instanceof Array) {
            let index = this._handers[event].findIndex(item => item == handler)
            if (index > -1) {
                this._handers[event].splice(index, 1);
            }
        }
    }
}